...
Commit messages must be in **Markdown format** with extensive emoji usage:

```markdown
🎉 ✨ Title summarizing changes (50-72 characters)

📝 Summary paragraph of all relevant changes (100-200 characters)

Detailed changes:

- 🐛 Fixed bug in Module X by updating function Y
- ✨ Added new feature Z to Component A
- 🔧 Updated configuration file .config
- 📝 Improved documentation in docs/Guide.md
- 🎨 Refactored code structure in src/Module/File.sh
...
1. **Title**: Concise summary (50-72 characters) with emojis
2. **Blank line** after title
3. **Summary paragraph**: 100-200 characters describing all changes
4. **Bullet points**: Detailed changes with emojis
5. **Group related changes** together
6. **Mention file names** when appropriate
7. **Explain the "why"** behind significant changes
8. **Reference issues**: Use `#123` format

### Special Rules

- **Ignore** files in `installScripts/` and `bin/` folders (they're generated)
- **Include every relevant change**, not just a summary
- **Use emojis liberally** in both title and bullet points

### Example Commit

```markdown
📦 ✨ Add new Python dependencies installer

🐍 Added comprehensive Python package management with dependency checking and
version validation.

Changes:

- ✨ Created src/_installScripts/Python/PythonDependencies.sh
- 📝 Added PythonDependencies-binary.yaml configuration
- 🧪 Implemented testInstall() and testConfigure() functions
- 🔧 Updated profile.default.sh to include PythonDependencies
- 📚 Updated docs/Install.md with Python setup instructions
- ✅ Added BATS tests in src/Python/PythonDependencies.bats
